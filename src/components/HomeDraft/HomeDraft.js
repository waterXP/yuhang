    ...
    if(approve && approve.list){
      ...
      if(type === 5) {
        list.map((cur,index,arr)=>{
          listHtml.push(
            <HomeRejectCell key={cur.expensesClaimsId} undoCell={cur} type={type} deleteExp={_this.props.deleteExp} />
          )
        })
      } else {
        list.map((cur,index,arr)=>{
          listHtml.push(
            <HomeDraftCell key={cur.expensesClaimsId} draftCell={cur} type={type} deleteExp={_this.props.deleteExp} />
          )
        })
      }
